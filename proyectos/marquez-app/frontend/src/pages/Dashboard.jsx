  ...
  return (
    <div className="space-y-4 max-w-4xl">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        <div className="kpi-card"><div className="text-xs text-gray-400 mb-1">Productos activos</div><div className="text-2xl font-semibold">{PRODUCTOS.length}</div><div className="text-xs text-green-600 mt-1">catálogo Marquéz</div></div>
          ...
          {Object.entries(catCount).sort((a, b) => b[1] - a[1]).map(([cat, cnt]) => (
            <div key={cat} className="mb-2">
              ...
              <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                <div className="h-full rounded-full" style={{ width: `${(cnt / PRODUCTOS.length) * 100}%`, background: '#BA7517' }} />
              </div>
            </div>
          ))}
        </div>
